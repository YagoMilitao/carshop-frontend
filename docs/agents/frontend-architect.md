...
- Organization of folders and files in `src/` (pages, components, hooks,
  services, types, etc.).
- Routing: Next.js App Router is the official pattern in use for routing
  decisions (see [docs/rules/routing.md](../rules/routing.md) and
...
- Current structure of `app/`/`src/` and existing configuration
  (`package.json`, `tsconfig*.json`) — reflects the repository's actual
  state, which is Next.js App Router (see
