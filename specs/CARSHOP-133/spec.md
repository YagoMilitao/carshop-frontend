...
estes 3 campos devem ser **opcionais** no schema (`z.string().url().optional()`),
já que o requisito é "renderiza somente se definida". O schema vazio hoje
existente em `lib/env/server.ts` deve ganhar esses 3 campos opcionais.
...
- `lib/env/server.ts` — adicionar os 3 campos opcionais ao
  `serverEnvSchema` e expor no objeto `serverEnv` exportado (ex.:
...
**SMALL** — escopo limitado a 4 arquivos já identificados
(`footer.tsx`, `footer.test.tsx`, `lib/env/server.ts`, `.env.example`),
...
- `architect`: não necessário — não há decisão de estrutura/rota/
  ...
  nesta spec.
- `plan-writer`: opcional (task é SMALL) — não obrigatório. Pode ser
  pulado, indo direto para `developer`, dado que o escopo já está bem
  delimitado nesta spec.
